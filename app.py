...
<available_functions>
{
  ...
  "buy_ticket": {
    "description": "Reserves a ticket for a movie at a specific theater and showtime. Requires the user to confirm the purchase.",
    "parameters": {
      "type": "object",
      "properties": {
        "theater": {
  ...
  "confirm_ticket_purchase": {
    "description": "Confirms a ticket purchase for a movie at a specific theater and showtime",
    "parameters": {
      "type": "object",
      "properties": {
        "confirmation_code": {
          "type": "string",
          "description": "The confirmation code for the ticket purchase"
        }
      },
...
async def prompt_model():
    ...
    if "<function_call>" in response_message.content:
        ...
        if function_call_json["name"] == "get_now_playing":
            ...
            print("buy_ticket")
            theater = function_call_json.get("arguments", {}).get("theater")
            movie = function_call_json.get("arguments", {}).get("movie") 
            showtime = function_call_json.get("arguments", {}).get("showtime")
            function_results = buy_ticket(theater, movie, showtime)
        elif function_call_json["name"] == "confirm_ticket_purchase":
            confirmation_code = function_call_json.get("arguments", {}).get("confirmation_code")
            function_results = confirm_ticket_purchase(confirmation_code)
